        ...
        for (int i = 0; i < args.size(); i++) { // flags in every even position: 0, 2, 4 etc...
            ...
            switch (arg) { // Increments the pointer
                case ArgTypes::ROUNDS:
                    GameConfig::rounds = ConfigParser::parseParam<int>(args[++i], "--rounds"); break;
                case ArgTypes::REPEATS:
                    GameConfig::repeats = ConfigParser::parseParam<int>(args[++i], "--repeats"); break;
                case ArgTypes::SEED:
                    GameConfig::seed = ConfigParser::parseParam<int>(args[++i], "--seed"); break;
                case ArgTypes::EPSILON:
                    GameConfig::epsilon = ConfigParser::parseParam<double>(args[++i], "--epsilon"); break;
                case ArgTypes::PAYOFFS:
                    GameConfig::payoffs = ConfigParser::validatePayoffs(args[++i]); break;
